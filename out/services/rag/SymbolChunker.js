...
class SymbolChunker {
     ...
     */
    async extractSymbols(fileContent, filePath) {
    ...
    walkTree(node, source, filePath, chunks, symbolTypes, isPython) {
    ...
    nodeToChunk(node, source, filePath, isPython) {
        const content = node.text;
        if (!content || content.length < 10)
            return null; // Skip trivially small nodes
    ...
    extractSymbolName(node, isPython) {
     ...
     */
    fallbackChunk(content, filePath, chunkSize = 500, overlap = 100) {
        const chunks = [];
...
exports.SymbolChunker = SymbolChunker;
//# sourceMappingURL=SymbolChunker.js.map